...
- Separation of concerns (detailed): The Flask app is an ops console. All conversational behavior—greetings, fallbacks, out-of-scope handling, latency decisions—must live inside each bot’s Rasa project (nlu/domain/stories/actions). The host code should only orchestrate crawling, indexing, training, model serving, and logging. No business logic or reply text belongs in the app layer.
